...
**SPEC-49i · Service PDP v2 (FROZEN 2026-08-05, founder redesign handoff `design_handoff_profile_booking` — Service PDP.dc.html + PATCHES §3/§4).**
- **Three price states, ONE resolver** — `src/lib/servicePricing.js` `priceForViewer(service, offering, viewerIsConnector)`: (1) **viewer-gated FREE** (`services.free_for_connectors` AND the viewer's own `cc_verified_at`; a creator sees free, everyone else sees the price — and can still request it free, the provider accepts/declines); (2) **service-wide discount** (`services.discount_pct` — one rate, EVERY offering, never per-offering); (3) plain price. Legacy $0 offerings still read free with the comparable price struck through (the 2026-05-30 rule).
- **Layout to the design**: hero white control circles (back/heart/pin/share) + mute, real story ruler only (>1 image); name 26 + full facet list (creator chip + FacetBadge); "Free for Local Creators" perk headline with viewer-aware gate copy; green go-to line + 46px lead avatar (one Link); "Book {First}" + state-aware info sub; 260px offering cards (Card r=18, check-for-free, struck was-price, green N%-off pill); mint custom-quote panel; About the provider; 213px go-to review cards (Card r=12, real reviewer avatars); "What people say" reco rows KEPT (SPEC-154 IG links + 49g badges); sticky bar = selected offering + viewer price + "Request {First}" (reply-offer/live-offer CTA labels unchanged — FW-7/SPEC-135).
- **"Leave a go-to review" REMOVED from the PDP** (PATCHES §4): reviews are post-booking only; the composer stays in the rate+post flow. The button was a nav shortcut to /inbox — nothing became unreachable.
- The recommenders select grew `avatar_url` (the PR-1 deferral, gate #159 amended in place); services select grew `free_for_connectors, discount_pct`; token `paper` (#FFFBF3, the handoff README's PDP bg) added. Zero raw hex in the migrated file. qa.mjs #263 pins this; #38/#53/#134/#136/#49g/#61 pin what stands.
...
**Status:** FROZEN — 2026-07-16 (Tarik, live testing). Four defects, each locked by a `spec-launch-1X-*` regression test in `scripts/qa.mjs`; ledger rows seeded by `supabase/migrations/20260716000000_req_launch_blockers_2.sql`. Bugs 12 + 13 are a privacy leak and a dead core loop.

1. **launch-12-address-isolation · A saved address is isolated PER USER.** The address localStorage cache MUST be namespaced by the signed-in user's auth id (`addrCacheKey(uid)` → `cergio.addr.<uid>`; guests use `cergio.guestAddress`). A single shared key leaked one account's saved address to the next account on the same browser (signed in as info@, the founder saw t@'s "134 Henry Street"). Every cache read/write in `HomeScreen` + `InlineLocationEditor` routes through the uid-scoped key; the durable truth is per-user `user_metadata` (`saveAddress`/`getDefaultAddress`). Two accounts can never share a cache entry, so one account can never surface another's address.
2. **launch-13-match-notify · A matching listed service IS notified.** The LISTING side (`ServiceListAboutScreen`) resolves a free-text service type through the SAME deterministic `resolveProviderTypeLocal` the REQUEST side uses (`resolvedProviderType = parserPT || canonicalMatch || localPT`). Before, a phrase like "french tutor" matched no exact catalog entry and — when the cloud resolver stalled — saved `taxonomy_provider_type = NULL`, invisible to `getProvidersForNotify` and `listServices`, while a "french tutor" SEARCH resolved locally to "Tutor". That request↔listing asymmetry was the dead core loop (searcher never reached the listed provider). The self-notify guard excludes ONLY the requester (`.filter(id => id !== uid)`), never a different matching provider.
3. **launch-14-list-route · List-a-service CTAs open the list flow.** `ClaimProfileScreen`'s "List your service" button navigated to `/services/manage` (the MANAGE screen) instead of `/list-service` (the list-a-service welcome flow). Fixed. `/list-service` (ServiceListWelcomeScreen) and `/services/manage` (ManageServicesScreen) stay distinct.
4. **launch-15-wait-copy-v2 · New post-request wait copy (SUPERSEDES launch-02).** The waiting state leads with heading **"We'll notify you when Connectors accept"** + body **"Your request is out to matching Connectors. Once one accepts, they show up here — free swaps first. Counter-offers land in Spotlight requests › Sent for you to accept or counter back."** + action **"Cancel request"**, keeping the pulsing live/working LeafLogo. Exported as `WAIT_HEADING` + `WAIT_COPY` (one source of truth). The launch-02 v1 single sentence is RETIRED (its `spec-launch-02` test is now the retirement guard for every old wait line).
