...
async function fulfillYelp(db: any, job: any): Promise<{ saved: number; found: number; query: string; note?: string }> {
  ...
  // Brickell/Wynwood/Doral are not Yelp locations.
  ...
  for (let offset = 0; offset < want && offset < 240; offset += 50) {   // yelp API caps offset at 240
    ...
    try {
      ...
      // SPEC-180: Yelp caps this endpoint at 240 businesses, so offset=200&limit=50 asked
      // for 250 and returned HTTP 400 — which then OVERWROTE the note of a run that had
      // already saved 200 rows. A last-page error must not erase a partial success.
...
async function fulfillYellowPagesApify(db: any, job: any): Promise<{ saved: number; found: number; query: string; note?: string }> {
  ...
  // trudax is the maintained equivalent (138k runs, ~79% success) and its input key is
  // `search`, NOT `keyword` — a wrong key returns an empty dataset that reads as an
  // empty market. 1000 results also cannot finish inside the run-sync window: the exact
  // SPEC-117 mistake, already fixed for gmaps and never applied here.
