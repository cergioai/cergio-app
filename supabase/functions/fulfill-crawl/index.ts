...
async function fulfillYelp(db: any, job: any): Promise<{ saved: number; found: number; query: string; note?: string }> {
  ...
  // SPEC-180: SPEC-170 metro normalisation was applied to craigslist/YP/LSA and never
  // to yelp — Brickell/Wynwood/Doral are not Yelp locations.
  ...
  for (let offset = 0; offset < want && offset < 240; offset += 50) {   // yelp API caps offset at 240
    ...
    try {
      ...
      // SPEC-180: Yelp caps this endpoint at 240 businesses, so offset=200&limit=50
      // asked for 250 and returned HTTP 400 — which then OVERWROTE the note of a run
      // that had already saved 200 rows. An error on the last page must not erase a
      // partial success.
...
async function fulfillYellowPagesApify(db: any, job: any): Promise<{ saved: number; found: number; query: string; note?: string }> {
  ...
  // trudax is the maintained equivalent — 138k runs, ~79% success — and its input key
  // is `search`, NOT `keyword`. We were also asking for 1000 results inside a run-sync
  // window that aborts at 140s: the exact mistake SPEC-117 already fixed for gmaps
  // with Math.min(want, 60) and never applied here.
  ...
  // Capture the reason NOW: _lastApifyError is a module-level global and the SPEC-169
  // pool runs 6 jobs at once, so a sibling apifyRun nulls it before we write our note.
