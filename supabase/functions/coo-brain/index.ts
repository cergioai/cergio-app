...
• "requires_approval" (boolean):
  ...
  - BLAST RADIUS (SPEC-89): a quarantine/relabel expected to move a LARGE number of rows — e.g. a whole-category sweep of the broad leads_services table (restaurant/retail/etc.), or any WHERE that is not narrowly scoped to a specific mislabeled cluster — MUST set requires_approval=true. Auto-run (requires_approval=false) is ONLY for narrow, targeted fixes. The executor now HARD-CAPS a single auto-run UPDATE at 2000 rows: a broader statement is rolled back to a no-op and force-gated to the founder anyway, so proposing it as auto-run just wastes a tick. When a sweep is genuinely needed, gate it.
