...
// "pending KEY" — so they are configured and running, and (b) the ONLY reason
// recorded was a generic "no X results", because the caller discarded the real
// apify/SerpAPI error. Declaring a free source dead without reading its own error
// is exactly the ceiling we do not accept. Every source is seeded again; the notes
// now carry the true reason (SPEC-168) so each can be fixed on evidence.
const SOURCES = [
  ['osm', 100],
  ['yelp', 240], ['craigslist', 1000], ['gmaps_apify', 60], ['yellowpages_apify', 1000],
  ['google_lsa', 1000], ['google_sponsored', 50], ['ig_services', 200],
];
